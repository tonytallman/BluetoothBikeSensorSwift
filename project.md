...
- **`Server` is built then started.** There is no public initializer. `Server: Sendable`. The initializer is `internal`. Session state lives on an internal `ServerLifecycle` actor and a per-cycle `ServerSession` actor.
...
- **Per-`Server` lifecycle (D5).** Each `Server` owns its own `ServerLifecycle` (`idle`, `starting`, `running`, `stopping`). A second `start()` on the same instance throws `.alreadyStarted`; separate `Server` values may run concurrently. `stopping` holds the teardown task so overlapping `stop()` calls wait on the same teardown.

- **Releasing a started `Server` stops it (D6).** `Server.deinit` schedules `stop()` in a task; apps that need deterministic teardown `await stop()` before releasing the last reference.
