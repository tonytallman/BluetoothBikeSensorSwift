...
- **`CSCServer` is a library product.** Phase 1 added the target and peripheral seam; Phase 2 added the builder; Phase 3 adds `start()` / `stop()` for crank-only and crank-plus-static-location shapes. `CSCClient` does not depend on `CSCServer`. `public start()` is the only production constructor of `CoreBluetoothPeripheral`; that instance is not passed into `Scanner`. Tests inject `FakeBluetoothPeripheral` through the package `start(peripheral:)` overload.
- **`Server` is built then started.** There is no public initializer. `Server: Sendable`. The initializer is `internal`. Session state lives on an internal `ServerRuntime` actor.
- **Supported `start()` shapes (Phase 3).** Crank-only and crank-plus-static location: no SC Control Point on the built service. Wheel, wheel-plus-static, crank-plus-multiple, wheel-plus-multiple, and wheel-plus-crank-plus-multiple throw `ServerError.unsupportedConfiguration` without touching the radio.
- **Phase 3 session behavior.** Subscribe-then-read on inbound peripheral streams before `add` / `startAdvertising`. Power-up waits up to 2 s (re-reads `currentState` at deadline). `public start()` lazily constructs one production peripheral and reuses it across sessions; package `start(peripheral:)` uses the injected fake per call. Feature and static-location reads are served from cached characteristic values; measurement reads return ATT `0x02`; writes return `0x03`. Crank notifications encode via `CSCWire` and honor notify backpressure through `subscriberUpdatesReady`. A single in-flight crank `next()` is not joined on teardown; stale pulls drop without notifying. Sequence `nil` ends the crank child with clean EOF; throws map to `revolutionSequenceFailed`. `stop()` and start-task cancellation tear down advertising and remove the service; `finishTeardown` resumes `start()` once with precedence cancel → recorded failure → startup/session failure → success.
- **Phase 3 session locks.** Per-arm episode cells; `onCancel` closes over that cell and must not resume a later arm. Power-wait `cancelAll` unblocks only that child and must not poison the park — the park returns session-ended and does not throw; a waiter `CancellationError` does not escape the session worker; `finishTeardown` resumes `start()` once. `cleanEOFSessionID` does not suppress the next session. After `add` and after `startAdvertising`, teardown-started or a recorded failure is the same early return as stop or cancel. `encode() == nil` stays defensive `revolutionSequenceFailed` with no test double. Restart and A→B subscribe while the previous `next()` is suspended drop that sample (empty `updateValue` once the new iterator is suspended, subscriber set still contains the central), then one new notify. The gated test sequence keeps one resume that arrives before `next()`; a second resume keeps the first and records a test issue.
...
- **`build()` stores configuration only.** `build()` copies multiple-location `supported` and `current`, stores sequences and delegates, and does not call delegates or iterate sequences. Phase 3 `start()` pulls crank data.
...
- **Queue crossing** — `CoreBluetoothPeripheral` creates `CBPeripheralManager` on serial queue `com.bluetoothbikesensor.peripheral`. The delegate bridge enqueues `Task { await handle(event) }` and returns. A pending-state latch is flushed on that queue at `bind` so `currentState` reflects the latest `didUpdateState` before reads. All manager calls run in `queue.sync` without holding the bridge lock across the sync. `add` and `startAdvertising` continuations resume on the actor. Only one in-flight `add` and one in-flight `startAdvertising` are allowed. `deinit` clears the handler and fails leftover continuations with `peripheralInvalidated` without calling `queue.sync`.
...
The type-state builder records CSCS feature bits, the GATT characteristic inventory, revolution sequences, and delegates. Phase 3 `start()` / `stop()` on crank-only and crank-plus-static-location configurations publish the service, advertise `0x1816`, serve feature and static-location reads, and notify crank measurements until stopped or cancelled.
