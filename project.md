...
- **Phase 4 `start()` scope.** Crank-only, crank-plus-static, wheel, wheel-plus-static, wheel-plus-crank, and wheel-plus-crank-plus-static. Any `.multiple` location throws `unsupportedConfiguration` before the radio is touched. A missing crank sequence is valid when wheel data is present.
...
- **Procedure idle until indication accept.** `procedureInProgress` stays set until the indication `updateValue` returns `true`, or shutdown or a throw discards it. A `false` return retries instead of ending the procedure.
...
#### CSC Server (Phase 2–4)

Phase 2 adds a type-state builder on `Server` that records CSCS feature bits, the GATT characteristic inventory, revolution sequences, and delegates. Phase 3 added crank `start()` / `stop()`. Phase 4 serves wheel configurations except `.multiple`, and Set Cumulative Value.
