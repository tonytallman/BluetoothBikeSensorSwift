...
- **Subscribe before `add` and `startAdvertising`** — `events` does not replay, so the session subscribes before `add`. Events that arrive during startup are buffered and handled in order once startup opens its gate, before the revolution loops start. Use `currentState` for the latest Bluetooth state.
